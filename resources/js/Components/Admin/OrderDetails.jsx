...
const OrderDetails = ({ order }) => {
  ...
  const updateStatus = () => {
    ...
    console.log(`Update order ${order.id} status to ${currentStatus} with notification: ${sendNotification}`);
          ...
          <div>
            {statusOptions.map(option => (
              option.value === order.status && (
                <span key={option.value} className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${option.color}`}>
                  {option.label}
                </span>
              )
            ))}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            {order.payment.transaction_id && (
              <div>
                ...
                <p className="text-sm text-gray-600">{order.payment.transaction_id}</p>
              </div>
            )}
            {order.payment.date && (
              <div>
                <p className="text-sm font-medium text-gray-700">Payment Date:</p>
                <p className="text-sm text-gray-600">{new Date(order.payment.date).toLocaleDateString()}</p>
              </div>
            )}
          </div>
        </div>
              ...
              <li key={index} className="flex space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {new Date(event.timestamp).toLocaleDateString()} at {new Date(event.timestamp).toLocaleTimeString()} by {event.user}
                  </p>
                </div>
              </li>
            ))}
